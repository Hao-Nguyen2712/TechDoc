...
TechDoc is implemented in .NET (ASP.NET Core), reusing the existing `Application` / `Core` / `Infrastructure` solution layout. Python (FastAPI + LlamaIndex/LangChain) was considered because of its richer RAG ecosystem, but rejected: the team knows .NET and prefers one language across API, ingestion pipeline, and tooling over ecosystem breadth.
